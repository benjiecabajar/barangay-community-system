...
const RequestCertificationModal = ({ isOpen, onClose, onSubmit, submissionStatus }) => {
  ...
  const isSubmitDisabled = () => {
    ...
    if (certType === 'Barangay Clearance') {
      const { firstName, lastName } = clearanceForm;
    ...
    if (certType === 'Certificate of Residency') {
      const { firstName, lastName, address, lengthOfResidency } = residencyForm;
      if (!firstName.trim() || !lastName.trim() || !address.trim() || !lengthOfResidency.trim() || !residencyFiles.govIdFront || !residencyFiles.govIdBack || !residencyFiles.utilityBill) {
        return true;
      }
    }
    ...
    if (certType === 'Certificate of Good Moral Character') {
      ...
      if (!govIdFront || !govIdBack || !proofOfResidency || !communityTaxCert) {
        return true;
      }
    }

    if (certType === 'Certificate of Indigency') {
      if (!indigencyFiles.front || !indigencyFiles.back) {
        return true;
      }
    }
